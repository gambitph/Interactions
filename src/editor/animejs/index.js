...
const round = ( v, decimalLength ) => {
	const m = 10 ** ( decimalLength || 0 ); return Math.round( v * m ) / m
}
...
/** @type {EasesFunctions} */
const eases = {
	linear, irregular, steps, cubicBezier,
}
	...
	if ( parent._hasChildren ) {
		...
		forEachChildren( animation, ( /** @type {Tween} */tween ) => {
			...
			if ( tweenTarget[ isDomSymbol ] ) {
				...
				} else if ( isUnd( originalInlinedValue ) || originalInlinedValue === emptyString ) {
				...
				if ( animation._tail === tween ) {
					animation.targets.forEach( ( /** @type {Target} */t ) => {
						if ( t.getAttribute( 'style' ) === emptyString ) {
...
class Animation extends Timer {
	...
	constructor( targets, parameters, parent, parentPosition, fastSet = false, index = 0, length = 0 ) {
		...
		const {
			delay, duration, ease, playbackEase, modifier, composition, onRender,
		} = params
...
const random = ( min, max, decimalLength ) => {
	const m = 10 ** ( decimalLength || 0 ); return floor( ( Math.random() * ( max - min + ( 1 / m ) ) + min ) * m ) / m
}
...
const shuffle = items => {
	let m = items.length - 1,
		t, i
	while ( m ) {
		i = random( 0, m-- ); t = items[ m ]; items[ m ] = items[ i ]; items[ i ] = t
	}
	return items
}
...
class Transforms {
	 ...
	 */
	traverseUp( cb ) {
		let $el = /** @type {DOMTarget} */( this.$el.parentNode ),
			i = 0
		while ( $el && $el !== doc ) {
			cb( $el, i )
...
export {
	Animatable, Animation, Clock, Draggable, Engine, Timeline, Timer, animate, createAnimatable, createDraggable, createTimeline, createTimer, defaults, eases, engine, spring, stagger, svg, utils,
}
